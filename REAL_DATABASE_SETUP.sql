-- Symbol Duel - Real Database Setup
...
CREATE TABLE IF NOT EXISTS public.profiles (
    ...
    email TEXT NOT NULL,
    avatar_url TEXT,
    account_balance DECIMAL(10,2) DEFAULT 100.00, -- $100 fake money on signup
...
CREATE TABLE IF NOT EXISTS public.games (
    ...
    game_data JSONB, -- Store puzzle data, scores, etc.
...
-- Create game_rounds table (for tracking individual puzzle rounds)
CREATE TABLE IF NOT EXISTS public.game_rounds (
    id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
    game_id UUID REFERENCES public.games(id) ON DELETE CASCADE,
    round_number INTEGER NOT NULL,
    puzzle_data JSONB NOT NULL, -- Store puzzle symbols, answer, etc.
    time_limit INTEGER DEFAULT 30,
    started_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    completed_at TIMESTAMP WITH TIME ZONE
);

-- Create player_answers table (for tracking individual answers)
...
BEGIN
    ...
    VALUES (
        ...
        NEW.email,
        100.00 -- $100 fake money on signup
    );
    RETURN NEW;
END;
